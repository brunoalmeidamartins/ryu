                            ...
                            for i in range(1,len(path_server_client)-1):
                                ...
                                for j in TABELA_MAC_SWITCH:
                                    if j[1] == switch_caminho:
                                        porta_switch = j[2]
                                        break
                                os.system('ovs-ofctl add-flow s' + str(switch_caminho) + ' priority=40000,dl_type=0x0800,nw_dst='+str(ip_client)+',nw_proto=17,idle_timeout=10,tp_dst='+str(porta_envio)+',actions=enqueue:'+str(porta_switch)+':'+str(fila_saida))
                                print('ovs-ofctl add-flow s' + str(switch_caminho) + ' priority=40000,dl_type=0x0800,nw_dst='+str(ip_client)+',nw_proto=17,idle_timeout=10,tp_dst='+str(porta_envio)+',actions=enqueue:'+str(porta_switch)+':'+str(fila_saida))
                                #Teste tempo
...
#app_manager.require_app('ryu.app.simple_switch_13_mod2')
