...
# Timeout for flows
FLOW_IDLE_TIMEOUT = 3		#IDLE_TIMEOUT alterado de 10 para 30
...
filename = '/home/bruno/pox/ext/classes.conf'
global classlist
global be
	...
	if net.protocol==17:
		...
		if transp.srcport==23232:
			...
			if net.tos!=0:
				...
				msg1.match.nw_tos=0
				msg1.match.tp_src=None
				msg1.match.tp_dst=23000
        ...
        if transp.dstport==23000:
            ...
            self.hasQoS[dpid][prt]+=classlist[self.portMap[net.dstip][1]].pico
            #Copia os parametros de cabecalho do pacote UDP
            log.info("MSG2 Deu aqui!!")
            msg2=of.ofp_flow_mod(command=of.OFPFC_MODIFY)
            msg2.match = of.ofp_match.from_packet(packet,inport)
            msg2.priority=65535
            msg2.idle_timeout=120
            msg2.hard_timeout=120
            msg2.match.nw_proto=17
            msg2.match.nw_tos=0
            msg2.match.tp_src=transp.srcport
            msg2.match.tp_dst=23000
            msg2.actions.append(of.ofp_action_enqueue(port=prt,queue_id=queue))
            #Instala a regra no roteador
